...
export default function TermsOfServiceEditor() {
	...
	const { loading, error, getTermsOfService, updateTermsOfService } = useAdminAppContent();
	...
	const handleSave = async () => {
		...
		const result = await updateTermsOfService(content);
		if (result.success) {
			setOriginalContent(content);
			setIsDirty(false);
			alert('Terms of service updated successfully!');
		} else {
			alert(result.error || 'Failed to update terms of service');
		}
	};
	...
	return (
		<Layout title="Terms of Service">
			<div className="space-y-6">
				<div className="flex justify-between items-center">
					<p className="text-muted-foreground">
						Edit the terms of service content using the rich text editor below
					</p>
			...
			<Modal
				...
				title="Terms of Service Preview"
				size="xl"
			>
				<div
